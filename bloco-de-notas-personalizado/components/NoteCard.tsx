...
const NoteCard: React.FC<NoteCardProps> = ({ note, onEdit, onDelete }) => {
  ...
  } else if (note.color.includes('-800')) { // For slate-800 or other potential -800 colors
    accentColor = note.color.replace('-800', '-600');
  } else if (note.color.includes('-700')) { // Just in case a -700 color is used directly
  ...
  return (
    <div className={`p-5 rounded-lg shadow-lg flex flex-col h-full transition-all duration-300 hover:shadow-xl ${note.color} relative overflow-hidden`}>
      ...
      <div className="mt-auto flex justify-end space-x-2 border-t border-slate-700/50 pt-3">
