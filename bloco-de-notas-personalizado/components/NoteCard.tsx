...
interface NoteCardProps {
  ...
  onEdit: () => void;
  onDelete: () => void;
  onExpand: () => void; // Nova propriedade para expandir
}

const NoteCard: React.FC<NoteCardProps> = ({ note, onEdit, onDelete, onExpand }) => {
  ...
  const sanitizeFilename = (name: string): string => {
    let sanitized = name.replace(/[^\w\s.-]/gi, '_'); 
    sanitized = sanitized.replace(/[\s_]+/g, '_'); 
    sanitized = sanitized.replace(/^_+|_+$/g, ''); 
    return sanitized.substring(0, 50) || 'nota'; 
  };
  ...
  return (
    <div className={`p-5 rounded-lg shadow-lg flex flex-col h-full transition-all duration-300 hover:shadow-xl ${note.color} relative overflow-hidden`}>
      ...
      <div className="mt-auto flex flex-wrap justify-end gap-x-2 gap-y-1 border-t border-slate-700/50 pt-3">
        <Button onClick={onExpand} variant="ghost" size="sm" className="focus:ring-offset-transparent text-sky-400 hover:text-sky-300">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 mr-1">
            <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9.75 9.75M20.25 3.75v4.5m0-4.5h-4.5m4.5 0L14.25 9.75M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9.75 14.25m10.5 6.001v-4.5m0 4.5h-4.5m4.5 0L14.25 14.25" />
          </svg>
          Expandir
        </Button>
